...
import re
from existing import Recommender
import traceback
...
def getRecommendation(model,groupedData,fold):
	...
	for user in groupedData:
		try:
			temp_data = groupedData[user]
			user_fields=['gender','zip_code','age_class','recency','rated']
			variable=['movie_id']
			user_field_values = [temp_data.iloc[0]['gender'],temp_data.iloc[0]['zip_code'],temp_data.iloc[0]['age_class'],1]
			evidences={x[0]:x[1] for x in zip(user_fields,user_field_values)}
			res=inference.query(variables=variable,evidence=evidences)
			result =(x for x in zip(res.state_names['movie_id'],res.values) if x[1])
			result = sorted(result,key=lambda x:x[1],reverse=True)
...
def testByUser(fold=False):
	...
	filename ='model_bayesian'+(str(fold) if fold else '')+".bat"
	filename = savePath+filename
	model = utility.loadModel(filename)
	important =['user_id','zip_code','occupation','gender','age_class','movie_id','genre','rated']
...
# 	'''
# 	# fold = translateFold(fold)
# 	filename ='model_bayesian'+(str(fold) if fold else '')
# 	model = utility.loadModel(filename)
...
def testByGenre(type=1,fold=False):
	...
	filename = savePath+filename
	model = utility.loadModel(filename)
	important =['user_id','zip_code','occupation','gender','age_class','movie_id','genre','recency','rated']
	testData = data.load_test_data(fold)
	testData = testData[important]
	testData = testData.dropna()
	testData['genre'] = testData['genre'].astype('int')
...
def runQueries():
	...
	try:
		test_number =int(input('Kindly specify the test you will like to run (1-4)'))

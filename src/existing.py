...
class Recommender:
	...
	def __init__(self,fold=False):
		self.variables=['movie_id','zip_code','occupation','gender','age_class','genre','rated','recency']
		# temp = ['a','b']
		self.fold = fold
	...
	def buildModel(self):
		filename = 'saved/model_existing'+str(self.fold if self.fold else '')+'.bat'
		if os.path.isfile(filename):
			self.model = utility.loadModel(filename)
			print('model loaded successfully')
			return self.model
		edges =[('movie_id','zip_code'),('movie_id','gender'),('movie_id','occupation'),('zip_code','age_class'),('age_class','recency'),('gender','occupation'),('age_class','occupation'),('recency','genre'),('occupation','genre'),('genre','rated')]
		self.model = BayesianModel(edges)
		movie_id,zip_code,gender,age_class,occupation,rated,genre_cpd,recency = tuple(self.getCPDs())

		# genre_cpd,context_cpd = tuple(self.getCPDs())

		self.model.add_cpds(movie_id,zip_code,gender,age_class,occupation,rated,genre_cpd,recency)
	...
	def getCPDs(self):
		...
		straightForward = ['movie_id','zip_code','gender','age_class','occupation','rated','genre','recency']
		cpds = []

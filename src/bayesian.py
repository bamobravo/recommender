...
class Recommender:
	...
	def __init__(self,fold=False):
		self.variables=['movie_id','zip_code','occupation','gender','age_class','genre','rated']
		# temp = ['a','b']
		self.fold = fold
	...
	def buildModel(self):
		filename = 'saved/model_bayesian'+str(self.fold if self.fold else '')+'.bat'
		if os.path.isfile(filename):
			self.model = utility.loadModel(filename)
			print('model loaded successfully')
			return self.model
		edges =[('movie_id','zip_code'),('movie_id','gender'),('movie_id','occupation'),('zip_code','age_class'),('gender','occupation'),('age_class','occupation'),('occupation','genre'),('genre','rated')]
		self.model = BayesianModel(edges)
		movie_id,zip_code,gender,age_class,occupation,rated,genre_cpd = tuple(self.getCPDs())

		# genre_cpd,context_cpd = tuple(self.getCPDs())

		self.model.add_cpds(movie_id,zip_code,gender,age_class,occupation,rated,genre_cpd)
	...
	def getCPDs(self):
		...
		straightForward = ['movie_id','zip_code','gender','age_class','occupation','rated','genre']
		cpds = []
